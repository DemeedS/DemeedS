<!-- ============ HEADER BANNER ============ -->
![header](https://capsule-render.vercel.app/api?type=waving&color=0:289620,100:1A3A5C&height=200&section=header&text=Demian%20%7C%20CS%20%2724%20%40%20Fordham&fontSize=50&fontColor=ffffff&fontAlignY=40&desc=Building%20AI%20agents%20and%20full-stack%20systems%20in%20NYC&descSize=16&descAlignY=70)

<!-- ============ ANIMATED TYPING SUBTITLE ============ -->
<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&duration=3000&pause=1000&color=289620&center=true&vCenter=true&width=600&lines=Full-Stack+Developer;AI+Agent+Builder;Back-End+Systems+Engineer)](https://git.io/typing-svg)

<!-- ============ SOCIAL BADGES ============ -->
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/demian-vial-8a748624a/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/DemeedS)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:YOUR-EMAIL@gmail.com)
[![Website](https://img.shields.io/badge/YourAgentCode-289620?style=for-the-badge&logo=googlechrome&logoColor=white)](https://youragentcode.com)

</div>

---

<!-- ============ ABOUT ============ -->
## 👋 About Me

Hey, I'm **Demian** — a B.S. Computer Science '27 student at **Fordham University** (Lincoln Center, NYC). I build **full-stack applications, back-end systems, and AI agents**.

I'm from **Saint Petersburg, Russia** 🇷🇺 — I moved to the US for high school without knowing a word of English. That taught me how to **learn fast and adapt under pressure**, and that mindset carries into everything I build.

- 🎓 Senior CS student · **3.5 GPA** · Dean's List
- 💼 IT Technician @ **Fordham EndPoint Services**
- 📦 e-Commerce Project Manager @ **Blithewold**
- 🤖 Founder @ **[YourAgentCode](https://youragentcode.com)** — AI automation for small businesses
- 🌍 Fluent in **Russian & English**
- ⚡ Fun fact: rock climber 🧗 · former all-state basketball 🏀 · cinematic content creator 🎬 (DaVinci Resolve)

---

<!-- ============ TECH STACK ============ -->
## 🛠️ Tech Stack

**Languages**

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white)

**Frameworks & Tools**

![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Claude API](https://img.shields.io/badge/Claude_API-D97757?style=flat-square&logo=anthropic&logoColor=white)

**Currently learning:** System Design · Data Structures & Algorithms · Agentic AI

---

<!-- ============ FEATURED PROJECTS ============ -->
## 📌 Featured Projects

| Project | Description | Stack |
| --- | --- | --- |
| 🤖 **[PulseAI](https://pulseai.onrender.com)** | Autonomous AI agent for org communication. One natural-language instruction → the agent creates the event, notifies members in real time, schedules reminders, detects conflicts, and collects RSVPs. Built & deployed in 24 hours. | `Python` · `Flask` · `Claude API` · `SocketIO` · `SQLite` |
| 🏦 **Bank Card System** | REST API for bank card management with JWT auth, role-based access control (RBAC), AES card encryption, and PostgreSQL. Built with real-world security patterns. | `Java` · `Spring Boot` · `PostgreSQL` · `JWT` |
| 👗 **Digital Wardrobe** | Collaborative Flask web app to organize & filter a personal wardrobe. Built the multi-filter dashboard (type, color, size, season) and favorites system with a Git branching workflow. | `Python` · `Flask` · `SQLAlchemy` |
| 💻 **DSA Projects** | C++ projects: linked lists, dynamic arrays, inheritance hierarchies, and OOP design patterns from coursework. | `C++` · `OOP` · `Data Structures` |

---

<!-- ============ GITHUB STATS ============ -->
## 📊 GitHub Stats

<div align="center">

![Demian's GitHub stats](https://github-readme-stats-sigma-five.vercel.app/api?username=DemeedS&show_icons=true&theme=chartreuse-dark&hide_border=true&count_private=true)

![Top Languages](https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=DemeedS&layout=compact&theme=chartreuse-dark&hide_border=true)

![GitHub Streak](https://streak-stats.demolab.com?user=DemeedS&theme=chartreuse-dark&hide_border=true)

</div>

---

<!-- ============ FOOTER BANNER ============ -->
![footer](https://capsule-render.vercel.app/api?type=waving&color=0:1A3A5C,100:289620&height=150&section=footer&text=Let%27s%20build%20something&fontSize=30&fontColor=ffffff&fontAlignY=65)
